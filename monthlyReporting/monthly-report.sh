...
psql -U dryad_app -d dryad_repo -c "\copy (select * from shoppingcart, item, metadatavalue where shoppingcart.item=metadatavalue.item_id and shoppingcart.item=item.item_id and item.in_archive=TRUE and metadatavalue.metadata_field_id=11) to '/tmp/shoppingArchived.csv' with CSV;"

echo Export blackout
psql -U dryad_app -d dryad_repo -c "\copy (select * from shoppingcart, item where shoppingcart.item=item.item_id and item.item_id in (select item_id from metadatavalue where text_value like '%Entered publication blackout%' and item_id not in (select item_id  from metadatavalue where text_value like '%Approved for entry into archive%')))  to '/tmp/shoppingBlackout.csv' with CSV;"
